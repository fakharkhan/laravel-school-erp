...
class ScaffoldViewCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'scaffold:view
     ...
     * @var string
     */
    protected $description = 'Create views for the Scaffold.';
    ...
    public function __construct()
    {
        ...
        $this->viewDirectoryPath = config('scaffoldgenerator.custom_template')
        ? config('scaffoldgenerator.path')
        : __DIR__ . '/../stubs/';
    }
    ...
    {
        $scaffoldName = $this->argument('name');
        $scaffoldNameCap = ucwords($scaffoldName);
        $scaffoldNameSingular = str_singular($scaffoldName);
        $modelName = ucwords($scaffoldNameSingular);
        ...
        if ($this->option('view-path')) {
            ...
            $path = $viewDirectory . $userPath . '/' . $scaffoldName . '/';
        } else {
            $path = $viewDirectory . $scaffoldName . '/';
        }
        ...
        foreach ($formFields as $key => $value) {
            ...
            if ($i == 0) {
                $formBodyHtml .= '<td><a href="{{ url(\'%%routeGroup%%%%scaffoldName%%\', $item->id) }}">{{ $item->' . $field . ' }}</a></td>';
            } else {
                $formBodyHtml .= '<td>{{ $item->' . $field . ' }}</td>';
            }
            $formBodyHtmlForShowView .= '<td> {{ $%%scaffoldNameSingular%%->' . $field . ' }} </td>';
        ...
        if (!File::copy($indexFile, $newIndexFile)) {
            ...
            File::put($newIndexFile, str_replace('%%scaffoldName%%', $scaffoldName, File::get($newIndexFile)));
            File::put($newIndexFile, str_replace('%%scaffoldNameCap%%', $scaffoldNameCap, File::get($newIndexFile)));
        ...
        // For _fields.blade.php file
        $createFile = $this->viewDirectoryPath . '_fields.blade.stub';
        $newCreateFile = $path . '_fields.blade.php';
        ...
        if (!File::copy($createFile, $newCreateFile)) {
            ...
            File::put($newCreateFile, str_replace('%%scaffoldName%%', $scaffoldName, File::get($newCreateFile)));
        ...
        if (!File::copy($editFile, $newEditFile)) {
            ...
            File::put($newEditFile, str_replace('%%scaffoldName%%', $scaffoldName, File::get($newEditFile)));
            File::put($newEditFile, str_replace('%%scaffoldNameSingular%%', $scaffoldNameSingular, File::get($newEditFile)));
